...
function parseLookbackDays(data: LooseObj): number {
...
function fillLookbackDaysField(days: number): void {
  const el = dialogEl?.querySelector("#schedule-lookback-days") as HTMLInputElement | null;
  if (el) el.value = String(days);
}

function readLookbackDaysField(): number {
  const el = dialogEl?.querySelector("#schedule-lookback-days") as HTMLInputElement | null;
