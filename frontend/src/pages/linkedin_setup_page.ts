...
export function bindLinkedinSetupInteractions(): void {
  ...
  const pullBtn = document.getElementById("linkedin-pull-btn") as HTMLButtonElement | null;
  ...
  pullBtn?.addEventListener("click", async () => {
    ...
    setStatus(
      ...
      useTrackedOnly
        ...
        : `Pulling ${keys.length} selected conversation(s) from LinkedIn… this may take a few minutes.`,
      "info",
    );
    ...
    try {
      ...
      setStatus(
        pullStatus,
        `Pulled ${data.message_count ?? 0} new message(s) from ${data.conversation_count ?? 0} conversation(s). Summaries are updating — refresh Threads shortly.`,
        "ok",
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
