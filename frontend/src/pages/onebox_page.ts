...
type MessageRow = ThreadView["messages"][number];
...
function messageDatetime(row: MessageRow): string {
  return str(row.cleaned?.datetime || row.summary?.datetime);
}

function messageSnippet(row: MessageRow): string {
...
function messageBody(row: MessageRow): string {
  const c = (row.cleaned || {}) as LooseObj;
  return str(c.cleaned_content) || str(c.raw_text) || "(No content)";
}

function messageSender(row: MessageRow): string {
  const c = (row.cleaned || {}) as LooseObj;
  return str(c.sender || c.forwarded_from);
}

function messageRecipients(row: MessageRow): string {
  const c = (row.cleaned || {}) as LooseObj;
  return formatRecipients(c.recipients);
}

function messageSubject(item: MessageItem): string {
  const c = (item.row.cleaned || {}) as LooseObj;
  const s = (item.row.summary || {}) as LooseObj;
  return str(c.subject || s.subject).trim() || threadEmailSubject(item.thread);
}

function messageKey(item: MessageItem): string {
  const c = (item.row.cleaned || {}) as LooseObj;
  const s = (item.row.summary || {}) as LooseObj;
...
type CalendarFields = { start?: string; end?: string; location?: string; attendees?: string; link?: string };

function parseCalendarFields(bodyText: string): CalendarFields {
  ...
  for (const rawLine of bodyText.split("\n")) {
    ...
    if (whenMatch) {
      ...
      if (end && end !== "(no end)") fields.end = end;
      continue;
    }
...
function calendarDetailsHtml(fields: CalendarFields): string {
  const rows: string[] = [];
  if (fields.start) {
    const startLabel = formatDate(fields.start);
    const endLabel = fields.end ? formatDate(fields.end) : "";
    rows.push(
      `<div class="calendar-field"><span class="calendar-field-label">Time</span><span class="calendar-field-value">${escapeHtml(startLabel)}${endLabel ? ` – ${escapeHtml(endLabel)}` : ""}</span></div>`,
    );
  }
  if (fields.location) {
    rows.push(
      `<div class="calendar-field"><span class="calendar-field-label">Location</span><span class="calendar-field-value">${escapeHtml(fields.location)}</span></div>`,
    );
  }
  if (fields.attendees) {
    rows.push(
      `<div class="calendar-field"><span class="calendar-field-label">Attendees</span><span class="calendar-field-value">${escapeHtml(fields.attendees)}</span></div>`,
    );
  }
  if (fields.link) {
    rows.push(
      `<div class="calendar-field"><span class="calendar-field-label">Link</span><a class="calendar-field-value calendar-field-link" href="${escapeHtml(fields.link)}" target="_blank" rel="noopener noreferrer">Open event</a></div>`,
    );
  }
  return rows.length ? `<div class="calendar-details">${rows.join("")}</div>` : "";
}

function laneMessagesSorted(data: LooseObj, laneId: number, threads: ThreadView[]): MessageItem[] {
...
function trackTabs(data: LooseObj): TrackTab[] {
  const threads = getCurrentThreads();
  const tabs: TrackTab[] = [];
  for (const lane of getLanes(data)) {
    if (Boolean(lane.archived) !== showArchivedTracks) continue;
...
function laneIdsToAutoUnarchive(data: LooseObj): number[] {
  const threads = getCurrentThreads();
  const out: number[] = [];
  for (const lane of getLanes(data)) {
    if (!lane.archived || lane.removed) continue;
    if (!lane.archived_at) continue;
    const items = laneMessagesSorted(data, lane.id, threads);
...
function unreadCount(tab: TrackTab): number {
...
function usesAreaGrouping(data: LooseObj): boolean {
  return getLaneAreas(data).length > 0 || getLanes(data).some((lane) => lane.area_id != null);
}

function areaGroups(data: LooseObj, tabs: TrackTab[]): AreaGroup[] {
...
function draftPanelHtml(item: MessageItem): string {
  ...
  const replyTo =
    threadChannelForThread(thread) === "email" ? extractEmailAddress(messageSender(item.row)) : "";
...
function planActionSuggestion(item: MessageItem): string {
...
function messageRowHtml(item: MessageItem): string {
  ...
  const snippetHtml = !isCalendar
    ...
    : "";
  const bodyHtml =
    !isCalendar && expanded ? `<pre class="onebox-row-body">${escapeHtml(messageBody(item.row))}</pre>` : "";
  const dirClass = messageDirectionClass(sender);
  const removeFromTrackBtn =
    activeTrackId != null
...
function renderOneboxTrackFilter(): void {
...
function renderOneboxToolbar(tabs: TrackTab[]): void {
...
function isUpcomingCalendarItem(item: MessageItem): boolean {
  if (threadChannelForThread(item.thread) !== "calendar") return false;
...
function formatUpcomingRelative(iso: string): string {
...
function timelineHtmlByDate(items: MessageItem[]): string {
  ...
  for (const item of items) {
    ...
    if (key !== lastKey) {
      ...
      if (label) parts.push(`<h3 class="onebox-section-heading onebox-date-heading">${escapeHtml(label)}</h3>`);
    }
...
function renderOneboxList(tabs: TrackTab[]): void {
...
async function renderOneboxOrThreadsView(): Promise<void> {
  ...
  if (oneboxViewMode === "threads") {
    for (const id of oneboxSectionIds) document.getElementById(id)?.setAttribute("hidden", "");
    threadsRoot?.removeAttribute("hidden");
    await renderDashboardThreadsInline();
    return;
  }
  threadsRoot?.setAttribute("hidden", "");
  for (const id of oneboxSectionIds) document.getElementById(id)?.removeAttribute("hidden");
  renderOnebox();
}

function renderOnebox(): void {
  const data = getCurrentData();
...
async function persistLaneThreadRemove(laneId: number, threadId: string): Promise<void> {
  const res = await fetch("/api/lanes/remove-thread", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ lane_id: laneId, thread_id: threadId }),
  });
  if (!res.ok) throw new Error(`Remove thread failed (${res.status})`);
}

async function persistLaneRemove(laneId: number): Promise<void> {
...
async function reloadOneboxFromServer(): Promise<void> {
  clearSummariesBundleCache();
...
async function autoUnarchiveNewActivity(data: LooseObj): Promise<boolean> {
  const laneIds = laneIdsToAutoUnarchive(data);
  if (!laneIds.length) return false;
  await Promise.all(
    laneIds.map((laneId) => persistLaneArchive(laneId, false).catch((err) => console.error(err))),
  );
  return true;
}
...
export function mountOneboxPage(root: HTMLElement): void {
  root.innerHTML = PAGE_HTML;
}

export async function renderOneboxPage(): Promise<void> {
  const data = getCurrentData();
...
export function bindOneboxInteractions(): void {
  ...
  document.addEventListener("click", (ev) => {
    ...
    const viewModeBtn = target.closest("[data-onebox-view-mode]") as HTMLButtonElement | null;
    if (viewModeBtn) {
      const mode = viewModeBtn.dataset.oneboxViewMode as "onebox" | "threads";
      if (!mode || mode === oneboxViewMode) return;
      oneboxViewMode = mode;
    ...
    const trackTab = target.closest(".onebox-track-tab") as HTMLButtonElement | null;
    if (trackTab) {
    ...
    if (addPlanBtn) {
      ...
      void (async () => {
        ...
        if (actionInput && !actionInput.value.trim() && suggestion) actionInput.value = suggestion;
      })();
      return;
    }

    const gmailReplyBtn = target.closest(".onebox-gmail-reply-btn") as HTMLButtonElement | null;
    if (gmailReplyBtn) {
      ...
      const draftOut = row?.querySelector<HTMLTextAreaElement>(".draft-markdown-output");
    ...
    if (draftToggle) {
      ...
      if (opening) draftPanelOpenKeys.add(threadId);
      else draftPanelOpenKeys.delete(threadId);
      renderOnebox();
      if (opening) {
        document
          .querySelector<HTMLTextAreaElement>(".draft-reply-panel:not([hidden]) .draft-intent-input")
          ?.focus();
      }
      return;
    }
    ...
    if (draftGen) {
      void (async () => {
        ...
        const intentEl = row?.querySelector<HTMLTextAreaElement>(".draft-intent-input");
        const intent = intentEl?.value.trim() ?? "";
        const outEl = row?.querySelector<HTMLTextAreaElement>(".draft-markdown-output");
        const errEl = row?.querySelector(".draft-reply-error") as HTMLElement | null;
    ...
    const rowToggle = target.closest(".onebox-row-toggle") as HTMLButtonElement | null;
    if (rowToggle) {
    ...
    const markToggleBtn = target.closest(".onebox-mark-toggle-btn") as HTMLButtonElement | null;
    if (markToggleBtn) {
    ...
    const markReadBtn = target.closest(".onebox-mark-read-btn") as HTMLButtonElement | null;
    if (markReadBtn) {
    ...
    const archiveBtn = target.closest(".onebox-archive-btn") as HTMLButtonElement | null;
    if (archiveBtn) {
    ...
    const trackFilterBtn = target.closest(".onebox-track-filter-btn") as HTMLButtonElement | null;
    if (trackFilterBtn) {
      const archived = trackFilterBtn.dataset.oneboxTrackFilter === "archived";
      if (archived === showArchivedTracks) return;
    ...
    const removeFromTrackBtn = target.closest(".onebox-remove-from-track-btn") as HTMLButtonElement | null;
    ...
    const removeBtn = target.closest(".onebox-remove-btn") as HTMLButtonElement | null;
    if (removeBtn) {
      const laneId = Number(removeBtn.dataset.trackId) || 0;
      if (!laneId) return;
      if (
        !window.confirm(
          "Remove this track from the onebox? It will stop being checked for new messages and can only be restored from Sources.",
        )
      ) {
        return;
      }
