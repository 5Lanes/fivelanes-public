...
function removedLanes(data: LooseObj | null): LaneView[] {
  return getLanes(data).filter((lane) => lane.removed);
}

function rowHtml(lane: LaneView): string {
...
async function restoreLane(laneId: number): Promise<void> {
...
export function mountRemovedTracksPanel(root: HTMLElement): void {
  root.innerHTML = PANEL_HTML;
}

export async function renderRemovedTracksPanel(): Promise<void> {
  renderList();
}

export function bindRemovedTracksInteractions(): void {
  if (interactionsBound) return;
  interactionsBound = true;

  document.addEventListener("click", (ev) => {
    const target = ev.target as HTMLElement | null;
    const btn = target?.closest(".removed-track-restore-btn") as HTMLButtonElement | null;
    if (!btn) return;
