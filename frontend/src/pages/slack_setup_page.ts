...
<div class="view-texts-setup view-slack-setup">
  <header class="texts-setup-header">
    ...
    <p class="texts-setup-lead">
      ...
      Tracked threads appear on the <a href="/onebox">onebox</a>.
    </p>
  </header>
...
function selectedKeys(): string[] {
  return Array.from(document.querySelectorAll<HTMLInputElement>(".slack-conversation-checkbox:checked"))
...
export function mountSlackSetupPage(root: HTMLElement): void {
  root.innerHTML = PAGE_HTML;
}

export async function renderSlackSetupPage(): Promise<void> {
...
export function bindSlackSetupInteractions(): void {
  if (interactionsBound) return;
  interactionsBound = true;

  const pullBtn = document.getElementById("slack-pull-btn") as HTMLButtonElement | null;
  const pullStatus = document.getElementById("slack-pull-status");
  const saveBtn = document.getElementById("slack-save-tracked-btn");
  const summarizeBtn = document.getElementById("slack-summarize-btn") as HTMLButtonElement | null;
  const saveStatus = document.getElementById("slack-save-status");
  ...
  pullBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      setStatus(
        pullStatus,
        `Pulled ${data.dm_count ?? 0} DM(s), ${data.message_count ?? 0} message(s).`,
        "ok",
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  selectAllBtn?.addEventListener("click", () => {
    document.querySelectorAll<HTMLInputElement>(".slack-conversation-checkbox").forEach((box) => {
      box.checked = true;
    });
    updateSelectionUi();
  });
