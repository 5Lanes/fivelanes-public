...
function selectedKeys(): string[] {
  return Array.from(document.querySelectorAll<HTMLInputElement>(".calendar-event-checkbox:checked"))
...
export function mountCalendarSetupPage(root: HTMLElement): void {
  root.innerHTML = PAGE_HTML;
}

export async function renderCalendarSetupPage(): Promise<void> {
...
export function bindCalendarSetupInteractions(): void {
  if (interactionsBound) return;
  interactionsBound = true;

  const saveBtn = document.getElementById("calendar-save-tracked-btn");
  const summarizeBtn = document.getElementById("calendar-summarize-btn") as HTMLButtonElement | null;
  const saveStatus = document.getElementById("calendar-save-status");
  ...
  selectAllBtn?.addEventListener("click", () => {
    document.querySelectorAll<HTMLInputElement>(".calendar-event-checkbox").forEach((box) => {
      box.checked = true;
    });
    updateSelectionUi();
  });
  ...
  summarizeBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      const data = (await res.json()) as {
        ...
        summarized?: number;
        skipped?: number;
        errors?: Array<{ dedupe_key?: string; error?: string }>;
      };
      if (!res.ok) {
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      setStatus(
        saveStatus,
        `Tracking ${data.tracked_count ?? keys.length} event(s). Summaries are generating in the background — refresh Threads in a minute.`,
        "ok",
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
