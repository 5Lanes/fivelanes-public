...
async function setEmailCapture(mode: EmailCaptureMode): Promise<EmailCaptureMode> {
...
function updateEmailCaptureUi(root: HTMLElement, mode: EmailCaptureMode): void {
  root.querySelectorAll<HTMLButtonElement>("[data-email-capture]").forEach((btn) => {
    const active = btn.dataset.emailCapture === mode;
    btn.classList.toggle("active", active);
    btn.setAttribute("aria-pressed", active ? "true" : "false");
  });
  const note = root.querySelector(".email-capture-labels-note") as HTMLElement | null;
  if (note) note.hidden = mode !== "labels";
}

function panelShell(forwardAddress: string): string {
...
export async function renderEmailCapturePanel(root: HTMLElement): Promise<void> {
  ...
  if (!root.dataset.mounted) {
    ...
    if (addrEl) addrEl.textContent = forwardAddress;
    else {
    ...
    const copyBtn = root.querySelector(".email-copy-address-btn") as HTMLButtonElement | null;
    if (copyBtn) copyBtn.disabled = !forwardAddress;
  }
  updateEmailCaptureUi(root, parseEmailCapture(data));
}

export function bindEmailCapturePanel(root: HTMLElement): void {
  if (panelBound) return;
  panelBound = true;

  root.addEventListener("click", (ev) => {
    const target = ev.target as HTMLElement;
    const captureBtn = target.closest("[data-email-capture]") as HTMLButtonElement | null;
    if (captureBtn && root.contains(captureBtn)) {
      const mode = captureBtn.dataset.emailCapture as EmailCaptureMode | undefined;
      if (!mode || captureBtn.classList.contains("active") || captureBtn.disabled) return;
      const errEl = root.querySelector(".email-capture-error") as HTMLElement | null;
