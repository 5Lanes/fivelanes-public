...
const SLOT_RE = /\b(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(\d{1,2})(?:,\s*(\d{4}))?(?:,\s*|\s+)(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\s*(?:-|–|to)\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)/gi;
const WEEKDAY_SLOT_RE = /\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\s+((?:morning|afternoon|evening)(?:\s*(?:\/|or|and)\s*(?:morning|afternoon|evening))*)/gi;
