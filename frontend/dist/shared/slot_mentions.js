...
function normalizeRange(start, end) {
    ...
    if (!start.meridiem && end.meridiem) {
        if (end.meridiem === "pm" && startMinute < 12 * 60)
            startMinute += 12 * 60;
    }
    ...
    if (endMinute < startMinute) {
        ...
        if (endMinute + 12 * 60 > startMinute)
            endMinute += 12 * 60;
    }
    if (endMinute <= startMinute)
        return null;
...
function chooseYear(month, day, yearRaw) {
    if (yearRaw)
    ...
    if (candidate.getTime() + windowMs < now.getTime())
...
const MONTH_WORD = "jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?";
const ORDINAL = "(?:st|nd|rd|th)?";
const TIME_TOKEN = String.raw `\d{1,2}(?::\d{2})?\s*(?:am|pm)?`;
const SLOT_RE = new RegExp(String.raw `\b(${MONTH_WORD})\s+(\d{1,2})${ORDINAL}(?:,\s*(\d{4}))?(?:,\s*|\s+|:\s*)(${TIME_TOKEN})\s*(?:-|–|to)\s*(${TIME_TOKEN})`, "gi");
const DATE_ANCHOR_RE = new RegExp(String.raw `\b(${MONTH_WORD})\s+(\d{1,2})${ORDINAL}(?:,\s*(\d{4}))?\b`, "gi");
const TIME_RANGE_RE = new RegExp(String.raw `\b(${TIME_TOKEN})\s*(?:-|–|to)\s*(${TIME_TOKEN})`, "gi");
const WEEKDAY_SLOT_RE = /\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\s+((?:morning|afternoon|evening)(?:\s*(?:\/|\bor\b|\band\b)\s*(?:morning|afternoon|evening))*)/gi;
...
function mentionFromTimeRange(text, startRaw, endRaw, absStart, absEnd, dateKey) {
...
function extractColonSeparatedSlots(text, asOf) {
    void asOf;
    const out = [];
    for (const dateMatch of text.matchAll(DATE_ANCHOR_RE)) {
        ...
        for (const timeMatch of scanText.matchAll(TIME_RANGE_RE)) {
            ...
            const mention = mentionFromTimeRange(text, String(timeMatch[1] || ""), String(timeMatch[2] || ""), scanFrom + relStart, scanFrom + relEnd, dateKey);
            if (mention)
                out.push(mention);
        }
    }
...
function parseTimeOfDayPhrase(phrase) {
    const parts = phrase
        .toLowerCase()
...
function nextWeekdayYmd(weekday, from = new Date()) {
