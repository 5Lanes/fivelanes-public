...
function summaryHasDisplayContent(s, isChat = false) {
    ...
    if (str(s.raw_text).trim())
        return true;
    if (!isChat && str(s.latest_status).trim())
        return true;
    if (arr(s.pending_items).some((x) => String(x).trim()))
        return true;
    return false;
}
function chatThreadSummaryScore(s) {
    let score = 0;
    if (arr(s.latest_updates).some((x) => String(x).trim()))
        score += 10;
    if (ownerOwnedNextSteps(s).length)
        score += 5;
    if (str(s.suggested_thread_label).trim())
        score += 1;
    return score;
}
export function threadSummaryForDisplay(thread) {
    ...
    for (const row of rows) {
        const s = (row.summary || {});
        if (summaryHasDisplayContent(s, isChat))
            return s;
    }
...
export function threadIsSlack(thread) {
    const primary = thread.messages[0] || { cleaned: null, summary: null };
    const c0 = (primary.cleaned || {});
    const s = threadSummaryForDisplay(thread);
...
export function latestUpdatesForThread(t) {
    ...
    if (!out.length && legacyStatus && !threadIsText(t) && !threadIsSlack(t))
    ...
    if ((threadIsText(t) || threadIsSlack(t)) && out.length) {
        const bodies = new Set(t.messages
            .map((row) => str(row.cleaned?.cleaned_content).trim())
        ...
        const filtered = out.filter((line) => {
            ...
            for (const body of bodies) {
                if (body.length >= 40 && norm.includes(body))
                    return false;
            }
            return true;
        });
...
export function shouldShowThreadMessageBlocks(thread, displayMessages) {
    if (threadIsText(thread) || threadIsSlack(thread))
        return false;
    if (displayMessages.length > 1)
        return true;
    return displayMessages.length === 1 && thread.messages.length > 1;
}
export function formatChatSenderLabel(sender) {
