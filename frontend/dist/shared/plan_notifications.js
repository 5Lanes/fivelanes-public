import { partitionPlansByDueStatus, planLinkedThreadLabel, } from "./plan_helpers.js";
...
function bannerSummary(overdue, dueToday) {
    const parts = [];
    if (overdue.length) {
...
function planListHtml(plans, heading) {
    if (!plans.length)
        return "";
    const items = plans
        .map((plan) => {
...
function updateNavBadge(overdue, dueToday) {
    const link = document.querySelector('.app-nav-link[data-route="plans"]');
    if (!link)
        return;
    let badge = link.querySelector(".nav-plan-badge");
...
function showDesktopPlanNotification(overdue, dueToday, opts = {}) {
    ...
    const fingerprint = [...overdue, ...dueToday]
        ...
        .sort()
        .join("|");
    if (!opts.force && sessionStorage.getItem(SESSION_NOTIFY_KEY) === fingerprint)
        return;
...
export function refreshPlanNotifications() {
