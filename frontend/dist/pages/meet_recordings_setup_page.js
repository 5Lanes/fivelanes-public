...
function selectedKeys() {
    return Array.from(document.querySelectorAll(".meet-document-checkbox:checked"))
...
export function mountMeetRecordingsSetupPage(root) {
    root.innerHTML = PAGE_HTML;
}
export async function renderMeetRecordingsSetupPage() {
...
export function bindMeetRecordingsSetupInteractions() {
    if (interactionsBound)
        return;
    interactionsBound = true;
    const pullBtn = document.getElementById("meet-pull-btn");
    const pullStatus = document.getElementById("meet-pull-status");
    const saveBtn = document.getElementById("meet-save-tracked-btn");
    const summarizeBtn = document.getElementById("meet-summarize-btn");
    const saveStatus = document.getElementById("meet-save-status");
    ...
    pullBtn?.addEventListener("click", async () => {
        ...
        try {
            ...
            await fetchCatalog();
            renderDocumentList();
            setStatus(pullStatus, `Found ${data.doc_count ?? 0} Meet recording doc(s). Select which to import below.`, "ok");
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    selectAllBtn?.addEventListener("click", () => {
        document.querySelectorAll(".meet-document-checkbox").forEach((box) => {
            box.checked = true;
        });
        updateSelectionUi();
    });
    ...
    saveBtn?.addEventListener("click", async () => {
        ...
        try {
            ...
            const errDetail = errs
                ...
                .join(" · ");
            setStatus(saveStatus, `Tracking ${data.tracked_count ?? keys.length} recording(s)` +
                (data.imported ? ` (imported ${data.imported} summary tab(s))` : "") +
                `. Summaries are generating in the background — refresh Threads in a minute.` +
                (errDetail ? ` Errors: ${errDetail}` : ""), errs.length && !(data.tracked_count ?? 0) ? "error" : "ok");
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            setStatus(saveStatus, msg, "error");
