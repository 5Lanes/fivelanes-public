...
function stepTypeClass(stepType) {
...
function threadSelectOptions(selectedId = "") {
...
function savedDraftForThread(threadId) {
...
function populateThreadSelect(selectedId = "") {
    const select = document.getElementById("plan-thread-select");
    if (!select)
        return;
    select.innerHTML = threadSelectOptions(selectedId);
}
function showAddPlanForm(selectedThreadId = "") {
...
function consumeThreadQueryParam() {
...
function openPlanEdit(card) {
    document.querySelectorAll(".plan-card.is-editing").forEach((other) => {
        if (other !== card)
            closePlanEdit(other);
    });
    ...
    draftPanel?.insertAdjacentHTML("beforebegin", planEditFormHtml({
        planId: item.planId,
        action: item.action,
    ...
    card.querySelector(".plan-edit-action-input")?.focus();
}
...
export function bindPlansInteractions() {
    ...
    document.addEventListener("click", (ev) => {
        ...
        const editCancel = target.closest(".plan-edit-cancel");
        if (editCancel) {
            const card = editCancel.closest(".plan-card");
            if (card)
                closePlanEdit(card);
            return;
        }
        const draftBtn = target.closest(".plan-draft-btn");
        if (draftBtn) {
            ...
            if (panel) {
                ...
                if (!panel.hidden)
                    panel.querySelector(".draft-intent-input")?.focus();
            }
            return;
        }
        const draftGen = target.closest(".draft-generate-btn");
        if (draftGen && draftGen.closest(".plan-card")) {
            void (async () => {
                ...
                const panel = card?.querySelector(`.plan-draft-panel[data-plan-key="${planKey}"]`);
                const intentEl = panel?.querySelector(".draft-intent-input");
                const intent = intentEl?.value.trim() ?? "";
                const outEl = panel?.querySelector(".draft-markdown-output");
                const errEl = panel?.querySelector(".draft-reply-error");
                if (!threadId || !outEl)
                    return;
                if (!intent) {
                    if (errEl) {
        ...
        const deleteBtn = target.closest(".plan-delete-btn");
        if (deleteBtn) {
    ...
    document.addEventListener("submit", (ev) => {
        const addForm = ev.target?.closest("#add-plan-form");
        if (addForm) {
            ev.preventDefault();
            void (async () => {
                const threadSelect = document.getElementById("plan-thread-select");
                const actionInput = document.getElementById("plan-action-input");
                const typeSelect = document.getElementById("plan-type-select");
                const whenInput = document.getElementById("plan-by-when-input");
        ...
        const editForm = ev.target?.closest(".plan-edit-form");
        if (!editForm)
            return;
        ev.preventDefault();
        void (async () => {
            const planId = Number(editForm.dataset.planId) || 0;
            const threadSelect = editForm.querySelector(".plan-edit-thread-select");
            const actionInput = editForm.querySelector(".plan-edit-action-input");
            const typeSelect = editForm.querySelector(".plan-edit-type-select");
            const whenInput = editForm.querySelector(".plan-edit-when-input");
            const threadId = threadSelect?.value.trim() ?? "";
            const action = actionInput?.value.trim() ?? "";
            const stepType = typeSelect?.value.trim() || "follow up needed";
            const byWhen = whenInput?.value.trim() ?? "";
