...
<div class="view-texts-setup">
  <header class="texts-setup-header">
    ...
    <p class="texts-setup-lead">
      ...
      Select which threads to track; tracked threads appear on the <a href="/dashboard">dashboard</a>.
    </p>
  </header>
...
function selectedKeys() {
    return Array.from(document.querySelectorAll(".texts-conversation-checkbox:checked"))
...
export function mountTextsSetupPage(root) {
    root.innerHTML = PAGE_HTML;
}
export async function renderTextsSetupPage() {
...
export function bindTextsSetupInteractions() {
    if (interactionsBound)
        return;
    interactionsBound = true;
    const saveBtn = document.getElementById("texts-save-tracked-btn");
    const summarizeBtn = document.getElementById("texts-summarize-btn");
    const saveStatus = document.getElementById("texts-save-status");
    ...
    selectAllBtn?.addEventListener("click", () => {
        document.querySelectorAll(".texts-conversation-checkbox").forEach((box) => {
            box.checked = true;
        });
        updateSelectionUi();
    });
