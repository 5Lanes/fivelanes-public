...
import { bindRemovedTracksInteractions, mountRemovedTracksPanel, renderRemovedTracksPanel, } from "./removed_tracks_panel.js";
const SOURCES_TAB_KEY = "fivelanes_sources_tab";
const VALID_SOURCES = ["email", "texts", "slack", "linkedin", "meet", "calendar", "removed"];
const PAGE_HTML = `
...
function sourceFromLocation() {
    ...
    if (VALID_SOURCES.includes(hash))
        return hash;
    try {
        const stored = localStorage.getItem(SOURCES_TAB_KEY);
        if (stored && VALID_SOURCES.includes(stored))
            return stored;
    }
    catch {
        /* ignore */
...
function sourceTabEnabled(source) {
    if (source === "email")
        return true;
    if (source === "texts")
        return isFeatureEnabled("texts");
    if (source === "slack")
...
function setActiveSourceTab(source) {
    ...
    document.querySelectorAll(".view-sources .source-tab-panel").forEach((panel) => {
        const active = panel.dataset.source === source;
...
async function ensureSourcePanels() {
...
async function refreshSourcePanel(source) {
    if (source === "email") {
        ...
        if (panel)
            await renderEmailCapturePanel(panel);
        return;
    }
...
function bindSourcesTabBar() {
    ...
    document.querySelector(".view-sources .source-tab-bar")?.addEventListener("click", (ev) => {
        ...
        if (!source || !sourceTabEnabled(source))
            return;
...
export async function showSourceTab(source) {
    if (!sourceTabEnabled(source))
        source = "email";
    await ensureSourcePanels();
    setActiveSourceTab(source);
    await refreshSourcePanel(source);
}
export function mountSourcesPage(root) {
...
export function bindSourcesInteractions() {
