...
function buildThreadCard(thread) {
    ...
    const cLatest = (latestDisplay.cleaned || {});
    ...
    const savedDraft = drafts[thread.id];
...
function renderCards(threads) {
    ...
    const renderBatch = (batchSize) => {
        ...
        if (index < threads.length)
            scheduleRemainingCards();
    };
    const scheduleRemainingCards = () => {
        if (token !== cardsRenderToken)
            return;
...
function renderNav(threads, snoozedCount, removedCount, channelCounts) {
    ...
    const channelOptions = [{ id: "all", label: "All", count: channelCounts.all }];
...
function observeThreadCard(card) {
    ensureNavObserver();
...
async function persistThreadSnooze(threadId, snoozed) {
    ...
    if (!res.ok)
        throw new Error(`Snooze save failed (${res.status})`);
}
async function persistThreadRemove(threadId) {
    ...
    if (!res.ok)
        throw new Error(`Remove tracking failed (${res.status})`);
}
async function persistThreadUnremove(threadId) {
    const res = await fetch("/api/thread-tracking/snooze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export function bindThreadsInteractions() {
    ...
    document.addEventListener("click", (ev) => {
        ...
        if (draftToggle) {
            ...
            const panel = card?.querySelector(".draft-reply-panel");
            if (panel) {
        ...
        if (draftGen) {
            void (async () => {
                ...
                const intentEl = card?.querySelector(".draft-intent-input");
                const intent = intentEl?.value.trim() ?? "";
                const outEl = card?.querySelector(".draft-markdown-output");
                const errEl = card?.querySelector(".draft-reply-error");
                if (!threadId || !outEl)
                    return;
                if (!intent) {
                    if (errEl) {
        ...
        const removeBtn = target.closest("button[data-remove-thread-id]");
        if (removeBtn) {
            ...
            if (!primary.summary)
                primary.summary = {};
            const currentlyRemoved = Number(primary.summary.snoozed || 0) === 2;
            primary.summary.snoozed = currentlyRemoved ? 0 : 2;
            reloadFromStore();
        ...
        const button = target.closest("button[data-snooze-thread-id]");
        if (!button)
            return;
        ...
        if (!primary.summary)
            primary.summary = {};
        const currentlySnoozed = Number(primary.summary.snoozed || 0) === 1;
        primary.summary.snoozed = currentlySnoozed ? 0 : 1;
        reloadFromStore();
