...
function renderMessages(history) {
    ...
    if (!history.length) {
        ...
        messagesEl.innerHTML = history
            .map((turn) => `<div class="aifred-chat-msg aifred-chat-msg-${turn.role}"><div class="aifred-chat-bubble">${escapeHtml(turn.content)}</div></div>`)
            .join("");
    }
...
export function mountAifredChat() {
    if (document.getElementById("aifred-chat-panel"))
        return;
  ...
  `;
    ...
    panelEl = panel;
    messagesEl = panel.querySelector("#aifred-chat-messages");
    formEl = panel.querySelector("#aifred-chat-form");
    inputEl = panel.querySelector("#aifred-chat-input");
    sendBtn = panel.querySelector("#aifred-chat-send");
