"""Run LLM thread summaries for tracked LinkedIn conversations and persist to SQLite."""
...
from services.thread_snooze import maybe_unsnooze_linkedin_thread
...
def summarize_one_linkedin_thread(
    db_path: str,
    ...
    *,
    ...
    maybe_unsnooze_linkedin_thread(db_path, key)
...
def summarize_tracked_linkedin_threads(
    db_path: str,
    *,
    ...
    for key in keys:
        try:
            result = summarize_one_linkedin_thread(
                db_path, key, force=force, run_stamp=run_stamp
            )
        except Exception as exc:
            log.exception("LinkedIn summary failed for %s", key)
