"""Run LLM thread summaries for tracked text conversations and persist to SQLite."""
...
def summarize_one_text_thread(
    ...
    *,
    ...
    """Segment-free summarize: cleaned bodies come from ``conversations/*.json``."""
    from utils.api_error_detection import thread_summary_is_valid
...
def summarize_tracked_text_threads(
    db_path: str,
    *,
    ...
    for key in keys:
        try:
            result = summarize_one_text_thread(
                db_path, key, force=force, run_stamp=run_stamp
            )
        except Exception as exc:
            log.exception("Text summary failed for %s", key)
